...
class SpeechChunkEncoder {
    ...
    fun encode(pcmFile: File, outputBase: File): Result {
        return if (supports(MediaFormat.MIMETYPE_AUDIO_OPUS)) {
            ...
            encodeWithCodec(pcmFile, out, MediaFormat.MIMETYPE_AUDIO_OPUS, 24_000, MediaMuxer.OutputFormat.MUXER_OUTPUT_OGG)
            Result(out, "opus", "audio/ogg")
        } else {
            val out = File(outputBase.parentFile, outputBase.nameWithoutExtension + ".m4a")
            encodeWithCodec(pcmFile, out, MediaFormat.MIMETYPE_AUDIO_AAC, 32_000, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4)
            Result(out, "aac", "audio/mp4")
        }
    }

    private fun supports(mime: String): Boolean = MediaCodecList(MediaCodecList.REGULAR_CODECS).codecInfos.any { info ->
        info.isEncoder && info.supportedTypes.any { it.equals(mime, ignoreCase = true) }
    }

    private fun encodeWithCodec(pcmFile: File, outputFile: File, mime: String, bitrate: Int, muxerFormat: Int) {
                ...
                while (!outputEnded) {
                    if (!inputEnded) {
                        ...
                        if (inputIndex >= 0) {
                            ...
                            buffer.clear()
                            val read = input.read(scratch, 0, minOf(buffer.remaining(), scratch.size))
                            val pts = presentationTimeUs(pcmBytes)
                            if (read < 0) {
        ...
        check(outputFile.exists() && outputFile.length() > 0) { "Encoded chunk is empty" }
    }
