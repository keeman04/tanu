...
object PipelineFiles {
    fun meetingsRoot(context: Context): File = File(context.filesDir, "mai-meetings").apply { mkdirs() }
    fun meetingDir(context: Context, meetingId: String): File = File(meetingsRoot(context), meetingId).apply { mkdirs() }
    fun audioDir(context: Context, meetingId: String): File = File(meetingDir(context, meetingId), "audio").apply { mkdirs() }
    ...
    fun mergedAudio(context: Context, meetingId: String): File = File(audioDir(context, meetingId), "meeting.aac")
    fun sequenceFromName(name: String): Int = name.removePrefix("chunk-").substringBefore('.').toIntOrNull() ?: -1
