...
def test_health_idempotent_upload_and_expected_chunk_finalization():
    with TestClient(app) as client:
        health = client.get("/health")
        assert health.status_code == 200
        assert health.json()["status"] == "ok"

        meeting = {"id": "meeting-test-001", "title": "Long Meeting Test", "started_at_ms": 123456}
        ...
        final = client.post("/v1/meetings/meeting-test-001/finalize", json={"expected_chunks": 2})
        assert final.status_code == 202

        update = client.get("/v1/meetings/meeting-test-001/updates")
        assert update.status_code == 200
        assert update.json()["total_chunks"] == 1
        assert update.json()["pending_chunks"] == 2  # one uploaded + one expected-but-not-uploaded
        assert update.json()["expected_chunks"] == 2
